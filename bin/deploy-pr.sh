...
function create_github_deployment {
    ...
    GITHUB_DEPLOYMENT_API_RESPONSE=$(curl --silent --header "Authorization: token ${GITHUB_TOKEN}" --header "Content-Type: application/json" --header "Accept: application/vnd.github.ant-man-preview+json" --request POST --data "${DEPLOYMENT_DATA}" "https://api.github.com/repos/eleven-labs/eleven-labs.github.io/deployments")
